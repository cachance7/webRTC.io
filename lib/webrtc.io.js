...
module.exports.listen = function(port, options) {//{{{
    ...
    io.sockets.on('connection', function(socket, args) {
        ...
        //TODO: Set up personal room if operating in 'onewaymirror' mode
            var room = args.room || uuid.v1();
            iolog(socket.id + ' is host of room ' + room);
        ...
        function joinRoom(data){//{{{
            ...
            if(connectionsId.length == 0){
                ...
                /* NO; BAD -- CANT ASSUME FIRST CONNECTION IS HOST
